...
int main(int argc, char* argv[]) {
  ...
  while (!done) {
    ...
    Timer comm_timer;
    comm_timer.start();
    MPI_CHECK(MPI_Allgather(&local_size, 1, MPI_INT, global_sizes.data(), 1, MPI_INT,
                  MPI_COMM_WORLD));
    ...
    MPI_CHECK(MPI_Alltoall(send_counts.data(), 1, MPI_INT, recv_counts.data(), 1, MPI_INT,
                 MPI_COMM_WORLD));
    ...
    MPI_CHECK(MPI_Alltoallv(send_buffer.data(), send_counts.data(), send_displs.data(),
                  MPI_UINT32_T, recv_buffer.data(), recv_counts.data(),
    ...
    MPI_CHECK(MPI_Allreduce(&local_frontier_size, &global_frontier_size, 1, MPI_INT,
